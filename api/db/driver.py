...
class Driver:
    ...
    def find_plan_by_type(self, plan_type: str = None):
        if plan_type is None:
            return self.db["plan_descriptions"].find()

        result = self.db["plan_descriptions"].find_one({"plan_type": plan_type})
        if result:
            return result

        print(f"info: plan with type {plan_type} not found")
    ...
    def delete_plan_by_type(self, plan_type: PlanKind):
        try:
            result = self.db["plan_descriptions"].delete_one({"plan_type": plan_type})

            if result.deleted_count == 0:
                print(f"error: plan with type {plan_type} not found")
            else:
                print(f"info: deleted plan with type {plan_type}")
...
if __name__ == "__main__":
    ...
    if db.create_plan(plan):
        passed += 1
    print("------------------------------------------------------------\n\n")
    print("find plan by type")
    total += 1
    if db.update_plan(plan):
        passed += 1
    
    print("------------------------------------------------------------\n\n")
    total += 1
    if db.find_plan_by_type("Bronze"):
        passed += 1
    
    print("------------------------------------------------------------\n\n")
    total += 1
    if db.delete_plan_by_type("Bronze"):
        passed += 1
